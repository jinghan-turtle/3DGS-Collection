...
SplaTAM 使用匀速模型 $\small E_{t+1} = E_t + (E_t-E_{t-1})$ 初始化相机位姿，也就是说前两帧和当前帧的相机位姿态构成了等差数列，这一点在下面的 `initialize_camera_pose` 函数中是显而易见的。
